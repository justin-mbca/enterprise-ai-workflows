...
def test_produce_consume_flow(kafka_producer: KafkaProducer, kafka_consumer: KafkaConsumer) -> None:
    ...
    assert found, f"Test message not found. Produced: {test_data['patient_id']}, Consumed: {[m.get('patient_id') for m in messages]}"
...
def test_kafka_topic_creation() -> None:
    ...
    try:
        ...
        expected_topics = ['medical-devices-vitals', 'hl7-messages', 'fhir-resources']
